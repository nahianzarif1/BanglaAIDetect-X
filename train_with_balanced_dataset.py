"""
train_with_balanced_dataset.py

Train the model using the balanced dataset (human + new AI samples).
"""
...
def main():
    ...
    print(f"Loaded balanced dataset: {len(df)} rows")
    print(f"Label distribution:\n{df['label'].value_counts()}")
    
    # Split data (simple split for now, can be improved to topic-based)
    ...
    import json
    with open(os.path.join(cfg["paths"]["reports"], "balanced_dataset_metrics.json"), "w", encoding="utf-8") as f:
        json.dump({
            "model": "tfidf_word+char + stylometry + logreg (balanced dataset)",
            "validation": val_metrics,
            "test": test_metrics,
            "note": "Trained on balanced dataset with 5,992 samples (2,996 human + 2,996 AI from merged dataset)"
        }, f, indent=2, ensure_ascii=False)
    
    print("\n✓ Model saved to models/fusion_detector.joblib")
    print("✓ Metrics saved to results/reports/balanced_dataset_metrics.json")
